...
for tt = 1 : length(type)
  ...
  %----------------------------
    ...
    fname = ['wirtinger_guassian_' type{tt} '_eig.txt'];
    fid = fopen(fname,'a');
    fprintf(fid, ['Wirtinger -> gaussian, ' type{tt} ', eig_init, n: %d, m: %d, iter: %d, t: %g, relres: %g, relerr: %g\n'], n, m, iter, t, norm(abs(Azw).^2-y)/norm(y), ... 
    ...
    for iter = 1 : maxiter
      ...
      %zg = ATA\(A'*yzg);
      [zg,~] = pcg(ATA,A'*yzg,1e-12,n,[],[],zg);
    end
    t = toc;

    % output results of Gerchberg-Saxton
    fname = ['GerSax_guassian_' type{tt} '_eig.txt'];
    fid = fopen(fname,'a');
    fprintf(fid, ['GerSax -> gaussian, ' type{tt} ', eig_init, n: %d, m: %d, iter: %d, t: %g, relres: %g, relerr: %g\n'], n, m, iter, t, norm(abs(Azg).^2-y)/norm(y), ... 
    ...
    for bls = 1 : length(blockSS)
      ...
      for iter = 1 : maxiter
        maxrelres = 0;
        for r = 1 : nblocks
          H = AAT((r-1)*blockSize+1:min(r*blockSize,m),(r-1)*blockSize+1:min(r*blockSize,m));
          ...
          zbk = zbk + Asub'*(H\((Asubzbk./abs(Asubzbk)).*sqrt(ysub)-Asubzbk));
        end
      ...
      fprintf(fid, [alg_name ' -> guassian, ' type{tt} ', eig_init, n: %d, m: %d, iter: %d, t: %g, relres: %g, relerr: %g\n'], n, m, iter, t, norm(y-abs(A*zbk).^2)/norm(y), ... 
