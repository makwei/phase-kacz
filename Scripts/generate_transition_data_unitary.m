  ...
  %----------------------------
    ...
    if strcmp(type{tt},'real') == 1 % real
      ...
      for i = 1 : ceil(m/n)
        tmp = randn(n,n);
        [Q,~] = qr(tmp);
        A((i-1)*n+1:min(m,i*n),:) = Q(1:min(n,m-(i-1)*n),:);
      end
      y = abs(A*x).^2;
    else                            % complex
      ...
      A = zeros(m,n);
      for i = 1 : ceil(m/n)
        tmp = 1/sqrt(2)*randn(n,n) + 1i/sqrt(2)*randn(n,n);
        [Q,~] = qr(tmp);
        A((i-1)*n+1:min(m,i*n),:) = Q(1:min(n,m-(i-1)*n),:);
      end
      y = abs(A*x).^2 ;
    ...
    fid = fopen(fname,'a');
    fprintf(fid, ['Kaczmarz -> unitary, ' type{tt} ', eig_init, n: %d, m: %d, iter: %d, t: %g, relres: %g, relerr: %g\n'], n, m, iter, t, norm(y-abs(A*zk).^2)/norm(y), ... 
