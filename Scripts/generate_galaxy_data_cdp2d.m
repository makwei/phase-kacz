...
for ll = 1 : length(D)
  ...
  for num = 1 : num_tests
    ...
    fname = 'wirtinger_cdp2d_gal.txt';
    fid = fopen(fname,'a');
    diff = abs(Azw).^2-Y;
    fprintf(fid, 'Wirtinger -> cdp2d, eig_init, n1: %d, n2: %d, m: %d, mode: %d, iter: %d, t: %g, relres: %g, relerr: %g\n', n1, n2, L*n1*n2, n3,iter, t, norm(diff(:),'fro')/norm(Y(:),'fro'), ... 
    ...
    fname = 'GerSax_cdp2d_gal.txt';
    fid = fopen(fname,'a');
    diff = abs(Azg).^2-Y;
    fprintf(fid, 'GerSax -> cdp2d, eig_init, n1: %d, n2: %d, m: %d, mode: %d, iter: %d, t: %g, relres: %g, relerr: %g\n', n1, n2, L*n1*n2, n3, iter, t, norm(diff(:),'fro')/norm(Y(:),'fro'), ... 
    ...
    fname = 'block_kaczmarz_cdp2d_gal.txt';
    fid = fopen(fname,'a');
    diff = abs(A(zbk)).^2-Y;
    fprintf(fid, 'Block Kaczmarz -> cdp2d, eig_init, n1: %d, n2: %d, m: %d, mode: %d, iter: %d, t: %g, relres: %g, relerr: %g\n', n1, n2, L*n1*n2, n3,iter, t, norm(diff(:),'fro')/norm(Y(:),'fro'), ... 
